...
instance FromRow HeadingRow where
  fromRow = HeadingRow <$> SQL.field <*> SQL.field
...
initDatabase conn = do
  SQL.execute_ conn "CREATE TABLE IF NOT EXISTS heading(id INTEGER PRIMARY KEY,term VARCHAR NOT NULL,reading VARCHAR NULL,CONSTRAINT unique_heading UNIQUE (term,reading))"
  SQL.execute_ conn dictionarySchema
  SQL.execute_ conn "CREATE TABLE IF NOT EXISTS entry(id INTEGER PRIMARY KEY,glossary VARCHAR NOT NULL,definition_tags VARCHAR NOT NULL,term_tags VARCHAR NOT NULL,popularity INTEGER NOT NULL,rules VARCHAR NOT NULL,heading_id INTEGER NOT NULL REFERENCES heading ON DELETE RESTRICT ON UPDATE RESTRICT,dictionary_id INTEGER NOT NULL REFERENCES dictionary ON DELETE RESTRICT ON UPDATE RESTRICT)"
  SQL.execute_ conn "CREATE TABLE IF NOT EXISTS tag(id INTEGER PRIMARY KEY,name VARCHAR NOT NULL,category VARCHAR NOT NULL,sorting_order INTEGER NOT NULL,notes VARCHAR NOT NULL,popularity INTEGER NOT NULL,dictionary_id INTEGER NOT NULL REFERENCES dictionary ON DELETE RESTRICT ON UPDATE RESTRICT,CONSTRAINT unique_tag_name UNIQUE (name,dictionary_id))"
  SQL.execute_ conn "CREATE TABLE IF NOT EXISTS kanji(id INTEGER PRIMARY KEY,kanji VARCHAR NOT NULL,onyomi VARCHAR NOT NULL,kunyomi VARCHAR NOT NULL,tags VARCHAR NOT NULL,meanings VARCHAR NOT NULL,stats BLOB NOT NULL,dictionary_id INTEGER NOT NULL REFERENCES dictionary ON DELETE RESTRICT ON UPDATE RESTRICT,CONSTRAINT unique_kanji UNIQUE (kanji,dictionary_id))"
  SQL.execute_ conn "CREATE TABLE IF NOT EXISTS kanji_frequency(id INTEGER PRIMARY KEY,kanji VARCHAR NOT NULL,data VARCHAR NOT NULL,dictionary_id INTEGER NOT NULL REFERENCES dictionary ON DELETE RESTRICT ON UPDATE RESTRICT,CONSTRAINT unique_kanji_frequency UNIQUE (kanji,data,dictionary_id))"
  SQL.execute_ conn "CREATE TABLE IF NOT EXISTS media(id INTEGER PRIMARY KEY,path VARCHAR NOT NULL,content BLOB NOT NULL,dictionary_id INTEGER NOT NULL REFERENCES dictionary ON DELETE RESTRICT ON UPDATE RESTRICT,CONSTRAINT unique_path UNIQUE (path,dictionary_id))"
  SQL.execute_ conn "CREATE TABLE IF NOT EXISTS term_meta(id INTEGER PRIMARY KEY,term VARCHAR NOT NULL,mode VARCHAR NOT NULL,data VARCHAR NOT NULL,dictionary_id INTEGER NOT NULL REFERENCES dictionary ON DELETE RESTRICT ON UPDATE RESTRICT,CONSTRAINT unique_term_meta UNIQUE (term,mode,data,dictionary_id))"
  SQL.execute_ conn "CREATE INDEX IF NOT EXISTS heading_reading_ix ON heading(reading)"
  SQL.execute_ conn "CREATE INDEX IF NOT EXISTS entry_heading_ix ON entry(heading_id)"
  where
    dictionarySchema =
      mconcat
...
updateDictionaryCounts conn dId = do
  ...
  tagC <- queryCount "tag"
  SQL.execute conn updateStmt (termsC, metaC, tagC, dId)
  where
    ...
    queryCount table = do
      let q =
            Util.sformat
              ...
              (table, dId)
      [Only c] <- SQL.query_ conn (Query q)
      pure c
    updateStmt =
      mconcat $
        [ "UPDATE dictionary ",
...
findDictionaryId conn dictName = do
  SQL.query conn "SELECT id FROM dictionary WHERE name = ?" (Only dictName) >>= \case
    (Only dictId : _) -> pure (Just dictId)
    [] -> pure Nothing

getDictionaries :: Connection -> IO [(DictionaryId, Text)]
getDictionaries conn = SQL.query_ conn "SELECT id, name FROM dictionary"

getOrInsertHeading :: Connection -> Text -> Maybe Text -> IO Int64
getOrInsertHeading conn term reading = do
  SQL.query conn "SELECT id FROM heading WHERE term = ? AND reading = ?" (term, reading) >>= \case
    (Only hId : _) -> pure hId
    [] -> do
      SQL.execute conn "INSERT INTO heading (term, reading) VALUES (?,?)" (term, reading)
      SQL.lastInsertRowId conn
...
insertTermMeta conn dictId termMeta = do
  SQL.execute
    ...
    `catch` \e ->
      if SQL.sqlError e == ErrorConstraint
        && T.isPrefixOf "UNIQUE constraint failed" (SQL.sqlErrorDetails e)
        then
          putStrLn
            ( Util.strFormat
...
insertDictionary conn dict = do
  ...
  case dictIdMay of
    Nothing -> SQL.withTransaction conn $ do
  ...
  where
    findTerm i text =
      let sqlQuery =
       ...
       in do
            rows <- SQL.query conn (Query sqlQuery) (text, text)
  ...
  where
    findTermMeta i text =
      let sqlQuery =
       ...
       in do
            rs <- SQL.query conn (Query sqlQuery) (Only text)
            pure $ map (i,) rs
  ...
  where
    ...
    findTagMeta (DictionaryAndQueryRequest {..}) =
      let sql =
       ...
       in SQL.query conn (Query sql) (dqrQuery, dqrDictionary) <&> \case
